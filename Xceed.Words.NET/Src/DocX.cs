...
namespace Xceed.Words.NET
{
    ...
    public class DocX : Container, IDisposable
    {
        ...
        public void ExposeFormFieldBookmark(string fieldName)
        {
            foreach (Paragraph p in Paragraphs)
            {
                var formField = p.Xml
                .Descendants(XName.Get("ffData", w.NamespaceName))
                .Where(e =>
                    (string)e.Descendants(XName.Get("name", w.NamespaceName))
                        .Attributes(XName.Get("val", w.NamespaceName))
                        .FirstOrDefault() == fieldName)
                .FirstOrDefault();

                if (formField != null)
                {
                    var defaultValue = formField.Descendants(XName.Get("default", w.NamespaceName))
                        .Attributes(XName.Get("val", w.NamespaceName))
                        .FirstOrDefault();

                    p._runs.Where(r => r.Descendants().Any(d => d.Value.Contains(defaultValue.Value)))
                       .Remove();

                    p._runs.Where(r => r.Descendants().Any(d => d.Value.Contains("FORMTEXT")))
                        .Remove();


                    // replace ffData - aka form field.
                    var runs = HelperFunctions.FormatInput(fieldName, null);
                    formField.Parent.Parent.AddBeforeSelf(runs.First());
                    formField.Parent.Parent.Remove();
                }
            }
        }
